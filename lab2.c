...
/* Lab 2 - Instructions:
	...
	6. After fixing thSpheree errors, the first line of output should be:
...
int main (void) {
	...
	// added a comma between (x=4) and (y=3) 
	// added a new variable, cr = root cubed
	int x = 4 , y = 3, z = -10, result,cr;
	...
	result = y + z / x;
	//replaced the ' into a " and added a ) before the semicolon also added a .2 to truncate
	printf("\nresult of y + z / x is %d, NOT %.2f  \n", result, -1.75);

	// Get user input
	// added an opening parenthesis after the printf and added a ; at the end of the line
	printf("Enter radius: ");  		
        // added a terminating " for &d
	scanf("%d", &radius);     			

	// volume of a sphere = 4/3 * PI * r * r * r 
	// assigning my cr variable a value
	cr= pow (radius,3);
	volume = x * M_PI * cr /y; 
	...
	printf("\nSphere volume = (");
	//changed print -> printf    added a comma after the &d
	printf("%d ", x);  			 		
	printf(" / ");
	//changed print -> printf    added a comma after the &d
	printf("%d " ,  y);  			 		
	printf(" * ");
	// added a comma between the %f and PI
	printf("%f" , M_PI);
	printf(" * ");
	// added "%d", before the radius
	printf("%d", radius);
	printf(" * ");
	// added "%d", before the radius
	printf("%d", radius);
	...
	// added "%d", before the radius
	printf("%d", radius);
	// replaced period with semicolon
	printf(") = ");
	//replaced ' into "
	printf("%.2f\n" , volume);
